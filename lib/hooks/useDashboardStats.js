...
export function useDashboardStats() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const summary = await api.get('/transactions/dashboard-summary', {
        from, to, months: '6', locale: chartLocale, limit: '5',
      })
      setStats(summary.stats)
      setCategoryData(summary.categoryExpenses)
      setHistoryData(summary.history)
