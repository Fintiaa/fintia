...
export async function checkBudgetsAndCreateAlerts() {
  ...
  for (const budget of budgets) {
    ...
    const message = percentage >= 100
      ? `Has excedido tu presupuesto de ${budget.category_id}. Gastaste ${Math.round(percentage)}% del límite.`
      : `Estás cerca del límite en ${budget.category_id}. Has usado ${Math.round(percentage)}% de tu presupuesto.`
